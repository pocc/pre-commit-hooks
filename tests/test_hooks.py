...
class TestHooks:
    ...
    def setup_class(cls):
        ...
        cls.scenarios = []
        for test_type in [cls.run_shell_cmd, cls.run_integration_test]:  # cls.run_cmd_class is unnecessary
            for s in scenarios:
    ...
    def test_run(self, test_type, cmd, args, files, expd_output, expd_retcode):
        ...
        test_type(cmd, files, args, expd_output, expd_retcode)
        if use_temp_files:
            for f in files:
    ...
    @staticmethod
    def run_integration_test(cmd_class, _, args, target_output, target_retcode):
        ...
        3. Run `pre-commit run --all-files` against the files
        """
        # Tests happen in the test_repo dir
        if not os.getcwd().endswith("test_repo"):
            test_dir = os.path.join(os.getcwd(), "tests/test_repo")
        else:
            test_dir = os.getcwd()
        args = list(args)  # redeclare so there's no memory weirdness
        for arg in args:  # So that file args make sense when the directory changes to tests/test_repo
            if "tests" in arg:
                args[args.index(arg)] = arg.replace("tests", "..")
...
repos:
  - repo: https://github.com/pocc/pre-commit-hooks
    rev: 07fad6c
    hooks:
      - id: {cmd_class.command}
        args: {args}
"""
        with open(pre_commit_config_path, "w") as f:
            f.write(pre_commit_config)
        sp_child = sp.run(["pre-commit", "run", "--all-files"], cwd=test_dir, stdout=sp.PIPE, stderr=sp.PIPE)
        stderr_actual = sp_child.stderr
        utils.assert_equal(target_output, stderr_actual)
        assert target_retcode == sp_child.returncode

    @staticmethod
    def run_shell_cmd(cmd_class, files, args, target_output, target_retcode):
